...
const getTargetDate = numberOfWeeks => {
  ...
  if (typeof numberOfWeeks !== 'number') {
    const error = {
      error: 'dateHelper error: number of weeks must be a number',
    };
  ...
  if (numberOfWeeks > 3 || numberOfWeeks < 1) {
    const error = {
      error:
        'dateHelper error: number of weeks must be an integer between 1 and 3',
    };
    return errorLogger(error, errorLogDir, __dirname);
  }
  ...
  const targetDate = new Date(
    dateInMilliseconds + numberOfWeeks * 7 * 24 * 60 * 60 * 1000,
  );

...
const getExpiredCoupons = () => Coupon.find({
  $expr: {
    $and: [
...
const returnCouponsUnusedReservedAmount = (ids, session) => Coupon.updateMany(
  { _id: { $in: ids } },
  [{ $set: { reservedAmount: "$usedAmount" } }],
  { session },
);

module.exports = { getCoupons, getExpiredCoupons, returnCouponsUnusedReservedAmount };

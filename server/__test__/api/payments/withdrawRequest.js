      ...
      .end(async (err, res) => {
        ...
        request(app)
          ...
          .end(async (error, result) => {
            ...
            expect(adamAccountAfter.currentBalance).toBe(
              adamAccountBefore.currentBalance,
            );

            const withdrawRequestsAfter = await WithdrawRequest.find();
            expect(withdrawRequestsAfter.length).toBe(
              withdrawRequestsBefore.length + 1,
            );
            return done();
          });
      });

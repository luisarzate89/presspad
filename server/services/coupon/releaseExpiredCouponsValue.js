const { getExpiredCoupons, returnCouponsUnusedReservedAmount } = require("../../database/queries/coupon");
...
const releaseExpiredCouponsValue = session => async () => {
  ...
  if (expiredCoupons && expiredCoupons.length) {
    ...
    expiredCoupons.forEach(({ usedAmount, reservedAmount, organisationAccount }) => {
    ...
    queries = [
      ...queries,
      ...Object.entries(accountsIncrementMap).map(([id, returnedAmount]) => returnCouponsValueToOrg(
        { id, returnedAmount, session },
      )),
    ];
